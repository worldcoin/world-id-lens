...
import WLDSplit from '../out/WLDSplit.sol/WLDSplit.json' assert { type: 'json' }
...
async function deploySplit(semaphoreAddress) {
    const groupId = await ask('Semaphore group id: ')

    const spinner = ora(`Deploying WLDSplit contract...`).start()

    let tx = await wallet.sendTransaction({
        data: hexlify(
            concat([
                WLDSplit.bytecode.object,
                abi.encode(WLDSplit.abi[0].inputs, [semaphoreAddress, groupId]),
            ])
        ),
    })
    spinner.text = `Waiting for WLDSplit deploy transaction (tx: ${tx.hash})`
    tx = await tx.wait()
    spinner.succeed(`Deployed WLDSplit contract to ${tx.contractAddress}`)
...
async function main(poseidonAddress, ibtAddress, semaphoreAddress) {
    ...
    await deploySplit(semaphoreAddress)
}
